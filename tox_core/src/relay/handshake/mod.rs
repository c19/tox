...
pub fn create_client_handshake(client_pk: &PublicKey,
    ...
    #[allow(clippy::redundant_slicing)] let encrypted_payload = common_key.encrypt(&nonce, &serialized_payload[..]).unwrap();
...
pub fn handle_client_handshake(server_sk: &SecretKey,
    ...
    let nonce = crypto_box::generate_nonce(&mut rand::thread_rng());
    #[allow(clippy::redundant_slicing)] let server_encrypted_payload = common_key.encrypt(&nonce, &serialized_payload[..]).unwrap();
